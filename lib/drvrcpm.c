...
static dsk_err_t rcpmfs_adjust_size(RCPMFS_DSK_DRIVER *self, unsigned lrbc,
            const char *filename)
{
    struct stat st;
    long newsize;
 ...
 * be, we stat() it, get the real size, and tweak that. */
    ...
    RTRACE(("Adjust file size (%s) lrbc=%d\n", filename, lrbc));
        ...
        char *value)
{
    ...
    if (!strcmp(variable, "systracks") && atoi(value)) 
...
/* See how many bytes there are in the extent */
    extent_len = (dirent[15] + ((dirent[12] & exm) * 128)) * 128;
    if (dirent[13])
    {
...
printf("Rename '%s' as '%s'", realname, newname);
 ...
 * handle it.       if (!err) err = rcpmfs_write_dirent(self, entryno, 
 *                              new, newname);
 */
...
/* Last Record Byte Count tweaked. Only do this for first extent */
        if (old[0x0d] != new[0x0d] && (newextent == 0))
        {
            rcpmfs_cpmname(new, realname);
            err = rcpmfs_adjust_size(self, new[0x0d],
                    rcpmfs_mkname(self,realname));
        }
        /* TODO: Change to a file's allocation. rcpmfs_flush() will
